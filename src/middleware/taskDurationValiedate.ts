...
const taskDurationValidate = async (from: Date, to: Date, employee: any, taskId?: string) => {
  ...
  const tasksForDay = await Task.aggregate([
    {
      $match: {
        ...
        ...(taskId && { _id: { $ne: new mongoose.Types.ObjectId(taskId) } }), 
      },
    },
    {
      $group: {
...
export default taskDurationValidate;
